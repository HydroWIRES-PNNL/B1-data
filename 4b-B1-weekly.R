...
#
# Modified from the original rectifhydplus code developed by Sean Turner
...
2001:end_year %>%
        ...
        map(
          function(x) {
            ...
            plant_has_release_flow <- nrow(
              flow_gauge |> na.omit() |> filter(EIA_ID == EIA_ID_, year == yr)
            ) >=
              365
            ...
            if (
              ...
              message('Assigning flat to ', x[["EIA_ID"]][1])
              x %>%
            ...
            weekly_targets <- x %>%
              ...
              left_join(
                select(wk_seq, date, jweek, week_start),
                by = "date"
              ) %>%
              group_by(jweek, week_start) %>%
              summarise(
  ...
  left_join(
    all_flows |> distinct(HUC4, USGS_ID),
    by = join_by(HUC4)
...
bind_rows(
  ...
  rename(HUC4_flow_cfs = av_flow_cfs) -> weekly_final

b1_weekly = weekly_final %>%
  mutate(
  ...
  # filter(eia_id == 6163) %>%
  # ggplot(aes(jweek, p_avg, group = year)) +
  ...
  select(eia_id, plant, datetime, everything())

b1_weekly |>
  write_csv(paste0(output_dir, "/B1_weekly.csv"), na = "") -> shh


# validation
b1_weekly <- paste0(output_dir, 'B1_weekly.csv') |> read_csv()
